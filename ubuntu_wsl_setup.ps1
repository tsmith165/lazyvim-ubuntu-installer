...
$fontInstalled = (Get-ChildItem -Path "$env:USERPROFILE\AppData\Local\Microsoft\Windows\Fonts" -Filter "*JetBrainsMono*" -Recurse -ErrorAction SilentlyContinue)
if (-not $fontInstalled) {
    ...
    $fontInstallPath = "$env:USERPROFILE\AppData\Local\Microsoft\Windows\Fonts"

    Invoke-WebRequest -Uri $fontUrl -OutFile $fontZipPath
    Expand-Archive -Path $fontZipPath -DestinationPath $fontInstallPath
    Remove-Item $fontZipPath

    Write-ColorOutput "JetBrains Mono Nerd Font installed successfully." -ForegroundColor Green
} else {
    Write-ColorOutput "JetBrains Mono Nerd Font is already installed. Skipping..." -ForegroundColor Yellow
}
...
if (-not $wsl2Installed) {
    ...
    Invoke-WebRequest -Uri https://wslstores.blob.core.windows.net/wslblob/wsl_update_x64.msi -OutFile "$env:TEMP\wsl_update_x64.msi"
...
if (-not $ubuntuInstalled) {
    ...
    Start-Process "ms-windows-store://pdp/?ProductId=9DP6T9HMX0JV"
